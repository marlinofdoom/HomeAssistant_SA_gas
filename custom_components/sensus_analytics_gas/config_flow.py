"""Config flow for Sensus Analytics Natural Gas Integration."""
...
class SensusAnalyticsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Sensus Analytics Integration (Natural Gas)."""
    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            unique_id = f"{user_input[CONF_ACCOUNT_NUMBER]}_{user_input[CONF_GAS_METER_NUMBER]}"
            ...
            if valid:
                return self.async_create_entry(title="Sensus Analytics (Gas)", data=user_input)
            errors["base"] = "auth"
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Optional("gas_heat_content_factor", default=1.03): cv.positive_float,
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Optional(
                    "gas_heat_content_factor",
                    default=current_data.get("gas_heat_content_factor", 1.037),
                ): cv.positive_float,
                vol.Required(
                    "gas_service_fee",
                    default=current_data.get("gas_service_fee", 13.00),
                ): cv.positive_float,
            }
        )
